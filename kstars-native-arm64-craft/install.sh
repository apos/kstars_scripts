...
if [[ -n "$DMG" ]]; then
	...
	# work/build/{bin,kstars}) plus Craft's own install prefix are all named
	# kstars.app/KStars.app and get regenerated by every build — enumerate them
	# rather than hardcode a list that drifts between Craft/KStars versions.
